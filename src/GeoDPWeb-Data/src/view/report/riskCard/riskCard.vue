...
export default {
  ...
  computed: {
    tableData() {
      return this.$store.state.report.riskCard.tableData;
    },
    activeName() {
        ...
        [this.$util.appconst.DataStatusEnum.Approval]: {
          title: [
            ...
            },
            {
              ...
              label: "预警信号",
              render: r => (
                <span>{r.forcastSignal ? r.forcastSignal : "--"}</span>
              )
            },
            {
              type: "func",
  ...
  methods: {
    ...
    query() {
      ...
      this.$store.dispatch("report/riskCard/query", parmas).then(res => {
    ...
    view(row) {
      // this.$store.dispatch("report/riskCard/edit", row.id).then(res => {
      //   if (res.data.success) {
    ...
    edit(row) {
      this.$store.dispatch("report/riskCard/edit", row.id).then(res => {
        if (res.data.success) {
          let id = res.data.result.id;
          this.$router.push({ name: "report.editRiskCard", query: { id: id } });
        }
      });
    },
    add() {
      this.$router.push({ name: "report.addRiskCard" });
    },
    // 提交
    submit() {
      if (this.selectionList.length) {
        this.$confirm("是否提交至上级审核？", "提交", {
          ...
          .then(() => {
            ...
            this.$store.dispatch("report/riskCard/commit", parmas).then(res => {
              if (res.data.success) {
                this.$message({
                  type: "success",
                  message: "提交成功!"
    ...
    pullBack(row) {
      this.$confirm("是否撤销该审核单？", "撤销", {
        ...
        .then(() => {
          this.$store
            .dispatch("report/riskCard/cancelCommit", row.id)
            .then(res => {
              if (res.data.success) {
                this.$message({
                  type: "success",
    ...
    delData(row) {
      this.$confirm("是否删除该条信息？", "删除", {
        ...
        .then(() => {
          this.$store.dispatch("report/riskCard/delData", row.id).then(res => {
            if (res.data.success) {
              this.$message({
                type: "success",
                message: "删除成功!"
