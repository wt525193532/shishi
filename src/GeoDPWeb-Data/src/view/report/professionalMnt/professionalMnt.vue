<template>
  <div class="report-professionalMnt">
    <report-layout
      :tab="tab"
      :queryForm="queryForm"
      :query="queryBtn"
...
export default {
  ...
  computed: {
    tableData() {
      return this.$store.state.report.professionalMnt.tableData;
    },
    activeName() {
  ...
  methods: {
    ...
    query() {
      ...
      this.$store.dispatch("report/professionalMnt/query", parmas).then(res => {
    ...
    view(row) {
      this.$router.push({
        path: "/report/professionalMnt/view",
        query: { id: row.id, code: row.code }
      });
    },
    edit(row) {
      this.$router.push({
        path: "/report/professionalMnt/edit",
        query: { id: row.id }
      });
    },
    add() {
      this.$router.push({ name: "report.addProfessionalMnt" });
    },
    // 提交
    submit() {
      if (this.selectionList.length) {
        this.$confirm("是否提交至上级审核？", "提交", {
          ...
          .then(() => {
            ...
            this.$store
              .dispatch("report/professionalMnt/commit", parmas)
              .then(res => {
                if (res.data.success) {
                  this.$message({
                    type: "success",
    ...
    pullBack(row) {
      this.$confirm("是否撤销该审核单？", "撤销", {
        ...
        .then(() => {
          this.$store
            .dispatch("report/professionalMnt/cancelCommit", row.id)
            .then(res => {
              if (res.data.success) {
                this.$message({
                  type: "success",
    ...
    delData(row) {
      this.$confirm("是否删除该条信息？", "删除", {
        ...
        .then(() => {
          this.$store
            .dispatch("report/professionalMnt/delData", row.id)
            .then(res => {
              if (res.data.success) {
                this.$message({
                  type: "success",
