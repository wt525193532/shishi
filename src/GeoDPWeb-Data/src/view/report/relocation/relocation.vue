...
export default {
  ...
  computed: {
    tableData() {
      return this.$store.state.report.relocation.tableData;
    },
    activeName() {
  ...
  methods: {
    ...
    query() {
      ...
      this.$store.dispatch("report/relocation/query", parmas).then(res => {
    ...
    view(row) {
      // this.$store.dispatch("report/relocation/edit", row.id).then(res => {
      //   if (res.data.success) {
    ...
    add() {
      this.$router.push({ name: "report.addRelocation" });
    },
    // 提交
    submit() {
      if (this.selectionList.length) {
        this.$confirm("是否提交至上级审核？", "提交", {
          ...
          .then(() => {
            ...
            this.$store
              .dispatch("report/relocation/commit", parmas)
              .then(res => {
                if (res.data.success) {
                  this.$message({
                    type: "success",
    ...
    pullBack(row) {
      this.$confirm("是否撤销该审核单？", "撤销", {
        ...
        .then(() => {
          this.$store
            .dispatch("report/relocation/cancelCommit", row.id)
            .then(res => {
              if (res.data.success) {
                this.$message({
                  type: "success",
    ...
    delData(row) {
      this.$confirm("是否删除该条信息？", "删除", {
        ...
        .then(() => {
          this.$store
            .dispatch("report/relocation/delData", row.id)
            .then(res => {
              if (res.data.success) {
                this.$message({
                  type: "success",
