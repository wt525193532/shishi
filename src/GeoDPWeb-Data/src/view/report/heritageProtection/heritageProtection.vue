<template>
  <div class="report-heritageProtection">
    <report-layout
      :tab="tab"
      :queryForm="queryForm"
      :query="queryBtn"
...
export default {
  ...
  computed: {
    tableData() {
      return this.$store.state.report.heritageProtection.tableData;
    },
    activeName() {
  ...
  methods: {
    ...
    query() {
      ...
      this.$store
        .dispatch("report/heritageProtection/query", parmas)
        .then(res => {
    ...
    view(row) {
      this.$router.push({
        path: "/report/heritageProtection/view",
        query: { id: row.id, code: row.code }
      });
    },
    edit(row) {
      this.$router.push({
        path: "/report/heritageProtection/edit",
        query: { id: row.id, code: row.code }
      });
    },
    add() {
      this.$router.push({ name: "report.addHeritageProtection" });
    },
    // 提交
    async submit() {
      if (this.selectionList.length) {
        this.$confirm("是否提交至上级审核？", "提交", {
          ...
          .then(() => {
            ...
            this.$store
              .dispatch("report/heritageProtection/commit", parmas)
              .then(res => {
                if (res.data.success) {
                  this.$message({
                    type: "success",
    ...
    pullBack(row) {
      this.$confirm("是否撤销该审核单？", "撤销", {
        ...
        .then(() => {
          this.$store
            .dispatch("report/heritageProtection/cancelCommit", row.id)
            .then(res => {
              if (res.data.success) {
                this.$message({
                  type: "success",
    ...
    delData(row) {
      this.$confirm("是否删除该条信息？", "删除", {
        ...
        .then(() => {
          this.$store
            .dispatch("report/heritageProtection/delData", row.id)
            .then(res => {
              if (res.data.success) {
                this.$message({
                  type: "success",
      ...
      this.$refs.dialogForm.$refs.dialogForm.validate(async valid => {
        if (valid) {
          ...
          cancel = await this.$store
            .dispatch("report/heritageProtection/cancelGeosite", parmas)
            .then(res => {
              if (res.data.success) {
                this.$message({
                  type: "success",
