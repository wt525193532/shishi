...
export default {
  ...
  computed: {
    tableData() {
      return this.$store.state.report.pwcx.tableData;
    },
    activeName() {
  ...
  methods: {
    ...
    query() {
      ...
      this.$store.dispatch("report/pwcx/query", parmas).then(res => {
    ...
    view(row) {
      this.$router.push({
        path: "/report/pwcx/view",
        query: { id: row.id, code: row.code }
      });
    },
    edit(row) {
      this.$router.push({
        path: "/report/pwcx/edit",
        query: { id: row.id }
      });
    },
    add() {
      this.$router.push({ name: "report.addpwcx" });
    },
    // 提交
    submit() {
      if (this.selectionList.length) {
        this.$confirm("是否提交至上级审核？", "提交", {
          ...
          .then(() => {
            ...
            this.$store.dispatch("report/pwcx/commit", parmas).then(res => {
              if (res.data.success) {
                this.$message({
                  type: "success",
                  message:
    ...
    pullBack(row) {
      this.$confirm("是否撤销该审核单？", "撤销", {
        ...
        .then(() => {
          this.$store.dispatch("report/pwcx/cancelCommit", row.id).then(res => {
            if (res.data.success) {
              this.$message({
                type: "success",
                message: "撤销成功!"
    ...
    delData(row) {
      this.$confirm("是否删除该条信息？", "删除", {
        ...
        .then(() => {
          this.$store.dispatch("report/pwcx/delData", row.id).then(res => {
            if (res.data.success) {
              this.$message({
                type: "success",
                message: "删除成功!"
