<template>
  <div class="report-eliminateDanger">
    <report-layout
      :tab="tab"
      :queryForm="queryForm"
      :query="queryBtn"
...
export default {
  ...
  computed: {
    tableData() {
      return this.$store.state.report.eliminateDanger.tableData;
    },
    activeName() {
  ...
  methods: {
    ...
    query() {
      ...
      this.$store.dispatch("report/eliminateDanger/query", parmas).then(res => {
    ...
    view(row) {
      // this.$store.dispatch("report/eliminateDanger/edit", row.id).then(res => {
      //   if (res.data.success) {
    ...
    add() {
      this.$router.push({ name: "report.addEliminateDanger" });
    },
    // 提交
    submit() {
      if (this.selectionList.length) {
        this.$confirm("是否提交至上级审核？", "提交", {
          ...
          .then(() => {
            ...
            this.$store
              .dispatch("report/eliminateDanger/commit", parmas)
              .then(res => {
                if (res.data.success) {
                  this.$message({
                    type: "success",
    ...
    pullBack(row) {
      this.$confirm("是否撤销该审核单？", "撤销", {
        ...
        .then(() => {
          this.$store
            .dispatch("report/eliminateDanger/cancelCommit", row.id)
            .then(res => {
              if (res.data.success) {
                this.$message({
                  type: "success",
    ...
    delData(row) {
      this.$confirm("是否删除该条信息？", "删除", {
        ...
        .then(() => {
          this.$store
            .dispatch("report/eliminateDanger/delData", row.id)
            .then(res => {
              if (res.data.success) {
                this.$message({
                  type: "success",
