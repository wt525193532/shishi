        ...
        <ul class="el-upload-list el-upload-list--picture-card">
          ...
          >
            ...
            <span class="el-upload-list__item-actions">
              ...
              >
                <i class="el-icon-delete" @click="fileRemove(item)"></i>
              </span>
            </span>
          </li>
        </ul>
...
export default {
  ...
  methods: {
    init() {
      [...this.attch] = this.tagClassification();
      let attchIndex = this.attch.length;
      let tagIndex = this.fileOption.tag.length;
      if (attchIndex !== tagIndex) {
        let a = tagIndex - attchIndex;
        if (a > 0) {
          for (let i = 0; i < a; i++) {
            this.attch.push({
              fileList: []
    ...
    fileRemove(file) {
      ...
      if (this.attchIndex - 0 >= 0) {
        delObj = this.attch[this.attchIndex].fileList;
  ...
  created() {
    /* 初始化 */
    this.attch = this.fileOption.tag.map(() => {
      return {
        fileList: []
      };
    });
