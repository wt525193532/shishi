    ...
    <div v-else>
      ...
      <div class="gl-content-item">
        <div class="gl-btn-row">
          <span class="gl-second-title">险情报送</span>
...
export default {
  ...
  methods: {
    ...
    query() {
      ...
      this.$store
        .dispatch("dutyReport/dangerReport/queryItems", params)
        .then(res => {
    ...
    addForm() {
      ...
      this.$router.push({ query: { routeName: "新增险情报送" } });
    },
    edit(row) {
      this.formCanEdit = true;
      this.$router.push({ query: { id: row.id, routeName: "编辑险情报送" } });
    },
    check(row) {
      this.formCanEdit = false;
      this.$router.push({ query: { id: row.id, routeName: "查看险情报送" } });
    },
    deleteFun(row) {
      this.$confirm("确认删除吗？", "提示", {
        ...
        .then(() => {
          this.loading = true;
          this.$store
            .dispatch("dutyReport/dangerReport/deleteItem", row.id)
            .then(res => {
              if (res) {
      ...
      if (this.selectColumns.length == 0) {
        ...
        this.$confirm("是否确认上报？", "提示", {
          ...
          .then(() => {
            ...
            this.$store
              .dispatch("dutyReport/dangerReport/report", reportForm)
